...
class HashResolver:
    ...
    def hash(self, value: int | str) -> int:
        return int(value) if isinstance(value, int) else jenk_hash(value)
    ...
    def resolve_hash(self, value: int) -> str | None:
        return self.hash_to_name.get(int(value))

    def resolve(self, value: int | str) -> int | str:
        if isinstance(value, str):
            return value
        resolved = self.resolve_hash(value)
        return resolved if resolved is not None else int(value)

    def resolve_or_none(self, value: int | str) -> str | None:
        if isinstance(value, str):
            return value
        return self.resolve_hash(value)

    def matches(self, left: int | str, right: int | str) -> bool:
...
def resolve_hash(value: int) -> str | None:
    return _GLOBAL_HASH_RESOLVER.resolve_hash(value)


def resolve_name(value: int | str) -> int | str:
    return _GLOBAL_HASH_RESOLVER.resolve(value)


def hash_matches(left: int | str, right: int | str) -> bool:
