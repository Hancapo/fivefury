...
from .schema import (
    YMT_ROOT_NAMES as YMT_SCHEMA_NAMES,
)
...
YMT_ROOT_NAMES.update(YMT_SCHEMA_NAMES)
...
class Ymt(MetaResource):
    ...
    content: Any = None
    format: YmtFormat = YmtFormat.RSC
    ...
    def scenario_point_manifest(self) -> PsoNode | None:
        ...
        return (
            self.content
            if self.content_type is YmtContentType.SCENARIO_POINT_MANIFEST
            else None
        )
    ...
    def scenario_manifest(self) -> YmtScenarioPointManifest | None:
        return (
            self.content if isinstance(self.content, YmtScenarioPointManifest) else None
        )
    ...
    def scenario_point_region(self) -> Any:
        return (
            self.content
            if self.content_type is YmtContentType.SCENARIO_POINT_REGION
            else None
        )
    ...
    def ped_variation(self) -> Any:
        return (
            self.content if self.content_type is YmtContentType.PED_VARIATION else None
        )
    ...
    def streaming_request_record(self) -> YmtStreamingRequestRecord | None:
        return (
            self.content
            if isinstance(self.content, YmtStreamingRequestRecord)
            else None
        )
    ...
    def root_type_hash(self) -> int:
        ...
        if self.rbf is not None:
            return (
                C_MAP_PARENT_TXDS
                if self.rbf.name == YmtContentType.MAP_PARENT_TXDS.value
                else 0
            )
        return 0
    ...
    def _from_rbf_bytes(cls, data: bytes, *, source: str = "") -> Ymt:
        root = read_rbf(data)
        content: Any = root
        content_type = ymt_content_type(root_name=root.name)
        if content_type is YmtContentType.MAP_PARENT_TXDS:
    ...
    def from_bytes(cls, data: bytes, *, source: str = "") -> Ymt:
        ...
        return cls._from_meta_resource(
            MetaResource.from_bytes(data, source=source), source=source
        )

    @classmethod
    def from_meta(cls, meta: Meta, *, source: str = "") -> Ymt:
        return cls._from_meta_resource(
            MetaResource.from_meta(meta, source=source), source=source
        )
