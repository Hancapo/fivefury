...
def parameter_component_count(type_name: str | None) -> int:
    return {"float": 1, "float2": 2, "float3": 3}.get((type_name or "").strip().lower(), 4)
...
class ShaderDefinition:
    ...
    def pick_file_name(self, bucket: int | None = None) -> str | None:
        if bucket is not None and (matches := self.get_bucket_file_names(bucket)):
            return matches[0]
        return self.file_names[0] if self.file_names else None

    def get_file_name(self, value: int) -> str | None:
        return self._file_name_hashes.get(int(value))
...
class ShaderLibrary:
    ...
    def __post_init__(self) -> None:
        ...
        for shader in self.shaders:
            ...
            for file_name in shader.file_names:
                self._by_file_name.setdefault(file_name.lower(), shader)
    ...
    def get_shader(self, value: str | int) -> ShaderDefinition | None:
        ...
        target = int(value)
        return self._by_name_hash.get(target) or self._by_file_hash.get(target)

    def get_file_name(self, value: int) -> str | None:
        shader = self._by_file_hash.get(int(value))
        return shader.get_file_name(value) if shader is not None else None
    ...
    def resolve_shader(
        ...
        *,
        ...
        for value in (shader_name, shader_name_hash, shader_file_name, shader_file_hash):
            if value and (shader := self.get_shader(value)) is not None:
                return shader
        if render_bucket is not None:
            return next((shader for shader in self.shaders if shader.get_bucket_file_names(render_bucket)), None)
        return None


def _coerce_bool(value: str | None) -> bool:
    return value is not None and str(value).strip().lower() in {"1", "true", "yes"}
...
def _parse_parameter_default_value(type_name: str, defaults: dict[str, str]) -> float | tuple[float, ...] | None:
    if str(type_name).strip().lower() == "texture":
        return None
    components = [_coerce_default_component(defaults[key]) for key in ("x", "y", "z", "w") if key in defaults]
...
def _parse_parameter(item: ET.Element) -> ShaderParameterDefinition:
    ...
    return ShaderParameterDefinition(
        ...
        defaults=defaults,
        default_value=_parse_parameter_default_value(type_name, defaults),
    )
