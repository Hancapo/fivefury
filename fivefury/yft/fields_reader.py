...
from ..binary import f32 as _f32, i32 as _i32, u16 as _u16, u64 as _u64
...
def read_fragment_pointers(system_data: bytes) -> YftFragmentPointers:
    return YftFragmentPointers(
        ...
        damaged_drawable=_u64(system_data, DAMAGED_DRAWABLE_POINTER_OFFSET),
...
def read_fragment_state(system_data: bytes) -> YftFragmentState:
    return YftFragmentState(
        ...
        unbroken_elasticity=float(_f32(system_data, 0xD0))
        if len(system_data) >= 0xD4
        else 0.0,
        gravity_factor=float(_f32(system_data, 0xD4))
        if len(system_data) >= 0xD8
        else 0.0,
        buoyancy_factor=float(_f32(system_data, 0xD8))
        if len(system_data) >= 0xDC
        else 0.0,
        glass_attachment_bone=system_data[0xDC] if len(system_data) > 0xDC else 0,
        num_glass_pane_model_infos=system_data[0xDD] if len(system_data) > 0xDD else 0,
    )
