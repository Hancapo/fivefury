...
def _scene(positions: list[tuple[float, float, float]]) -> trimesh.Scene:
    scene = trimesh.Scene()
    scene.add_geometry(
        trimesh.Trimesh(
            vertices=numpy.asarray(positions, dtype=float),
            faces=((0, 1, 2),),
            process=False,
        ),
        geom_name="navmesh",
...
def test_trimesh_to_ynvs_writes_single_valid_ynv(tmp_path: Path) -> None:
    source = _scene(
        [
            (10.0, 0.0, -10.0),
            (20.0, 0.0, -10.0),
    ...
    outputs = trimesh_to_ynvs(source, tmp_path / "out")
