...
from fivefury import AngleMode, BoxOccluder
...
def test_box_occluder_matches_gta_v_runtime_layout() -> None:
    ...
    assert occluder.size == (8.0, 4.0, 6.0)
    ...
    assert occluder.bounds[0] == pytest.approx(
        (10.0 - extent_x, 20.0 - extent_y, 27.0),
        abs=1e-3,
    )
    assert occluder.bounds[1] == pytest.approx(
...
def test_box_occluder_preserves_sub_quantum_boxes_as_minimum_size() -> None:
    occluder = BoxOccluder.from_box((0.0, 0.0, 0.0), (0.1, 0.1, 0.1))

    assert occluder.size == (0.25, 0.25, 0.25)
