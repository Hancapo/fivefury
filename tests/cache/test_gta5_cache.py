    ...
    return Gta5CacheY(
        ...
        map_data=[
            Gta5CacheMapData(
                ...
                streaming_min=Vector3(-100.0, -100.0, -10.0),
                streaming_max=Vector3(100.0, 100.0, 50.0),
                physics_min=Vector3(-20.0, -30.0, 0.0),
                physics_max=Vector3(20.0, 30.0, 10.0),
        ...
        interior_proxies=[
            Gta5CacheInteriorProxy(
                ...
                bounds_min=Vector3(5.0, 15.0, 25.0),
                bounds_max=Vector3(15.0, 25.0, 35.0),
            )
        ],
        bounds=[
            Gta5CacheBound(
...
def test_binary_roundtrip_preserves_all_runtime_tables() -> None:
    ...
    assert rebuilt.interior_proxies[0].position.components == pytest.approx(
        (10.0, 20.0, 30.0)
    )
...
def test_builder_derives_map_mlo_and_bound_entries() -> None:
    parent = Ymap(
        ...
        streaming_extents_min=Vector3(-50.0, -50.0, -10.0),
        streaming_extents_max=Vector3(50.0, 50.0, 30.0),
        entities_extents_min=Vector3(-10.0, -10.0, 0.0),
        entities_extents_max=Vector3(10.0, 10.0, 20.0),
    )
    child = Ymap(
        name="child_map",
        parent="parent_map",
        ...
        streaming_extents_min=Vector3(90.0, 190.0, 0.0),
        streaming_extents_max=Vector3(130.0, 230.0, 50.0),
        entities_extents_min=Vector3(95.0, 195.0, 5.0),
        entities_extents_max=Vector3(125.0, 225.0, 45.0),
        entities=[
            MloInstanceDef(
                archetype_name="test_mlo",
                position=Vector3(100.0, 200.0, 10.0),
                group_id=3,
                floor_id=2,
            )
        ],
    ...
    archetype = MloArchetypeDef(
        name="test_mlo",
        bb_min=Vector3(-5.0, -10.0, 0.0),
        bb_max=Vector3(5.0, 10.0, 20.0),
        portals=[MloPortalDef(room_from=0, room_to=1)],
    )
    collision = BoundBox.from_bounds(
        Vector3(-15.0, -20.0, -2.0),
        Vector3(15.0, 20.0, 22.0),
    )
    ...
    assert cache.interior_proxies[0].bounds_min.components == pytest.approx(
        (95.0, 195.0, 5.0)
    )
    assert cache.interior_proxies[0].bounds_max.components == pytest.approx(
        (125.0, 225.0, 45.0)
    )
    assert cache.bounds[0].minimum == Vector3(-15.0, -20.0, -2.0)
...
def test_builder_expands_distant_lod_streaming_bounds_to_world_physics() -> None:
    distant = Ymap(
        ...
        streaming_extents_min=Vector3(-10.0, -10.0, -10.0),
        streaming_extents_max=Vector3(10.0, 10.0, 10.0),
        entities_extents_min=Vector3(-5.0, -5.0, -5.0),
        entities_extents_max=Vector3(5.0, 5.0, 5.0),
    )
    world = Ymap(
        name="world",
        streaming_extents_min=Vector3(-100.0, -200.0, -30.0),
        streaming_extents_max=Vector3(300.0, 400.0, 50.0),
        entities_extents_min=Vector3(-80.0, -150.0, -20.0),
        entities_extents_max=Vector3(250.0, 350.0, 40.0),
    )

    entry = build_gta5_cache_y([distant, world]).map_data[0]

    assert entry.streaming_min == Vector3(-80.0, -150.0, -20.0)
    assert entry.streaming_max == Vector3(250.0, 350.0, 40.0)
...
def test_dlc_cache_enforces_the_runtime_buffer_limit() -> None:
    cache = Gta5CacheY(
        ...
        bounds=[
            Gta5CacheBound(index + 1, Vector3(), Vector3(1.0, 1.0, 1.0))
