...
def _synthetic_manifest() -> PackFileMetaData:
    return PackFileMetaData(
        imap_dependencies_2=[
            ImapDependencies(0x100, [0x200], ManifestFlags.INTERIOR_DATA)
        ],
        ityp_dependencies_2=[
            ItypDependencies(
                0x200,
        ...
        interiors=[InteriorBoundsFile(0x200, [0x200])],
    )


def test_ymf_writer_uses_runtime_pso_layout() -> None:
    sections = parse_sections(build_ymf(_synthetic_manifest()).to_bytes())
...
def test_ymf_runtime_roundtrip_is_byte_identical() -> None:
    source = build_ymf(_synthetic_manifest()).to_bytes()
    ymf = read_ymf(source)
...
def test_ymf_runtime_validation_rejects_out_of_range_block_pointer() -> None:
    source = bytearray(build_ymf(_synthetic_manifest()).to_bytes())
    root_imap_array_pointer = 120 + 48
    source[root_imap_array_pointer : root_imap_array_pointer + 4] = (0xFFF).to_bytes(
        4, "big"
    )
...
def test_build_ymf_manifest_for_ymaps_resolves_archetypes_from_cache() -> None:
    ...
    manifest = build_ymf_manifest_for_ymaps([ymap], context=_build_context(cache=cache))
...
def test_prepared_ymf_dependency_index_is_reused_across_manifest_groups() -> None:
    ...
    cache = _CountingCache([(_FakeAsset("city_ityp", GameFileType.YTYP), ytyp)])
...
def test_ymf_validation_rejects_non_hour_bits_and_oversized_arrays() -> None:
    ...
    assert any(
        issue.code == "ymf.map_group.hours.range" for issue in invalid_hours.validate()
    )
